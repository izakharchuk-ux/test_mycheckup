    ...
    <div className="min-h-screen bg-[#F6FBFF] flex flex-col items-center justify-center px-5 py-10">
      <div className="w-full max-w-sm">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-[22px] font-extrabold text-[#0b2a3b]">Create account</h1>
          <p className="text-[13px] text-[#6b7785] mt-1">Start your health journey today</p>
        </div>

        <div className="flex flex-col gap-4">
          <div>
            <label className="text-[12px] font-semibold text-[#0b2a3b] mb-1.5 block">Full Name</label>
          ...
          <div>
            ...
            <input name="email" type="email" value={form.email} onChange={handle} placeholder="olena@email.com"
              className={`w-full border rounded-xl px-4 py-3 text-[14px] focus:outline-none bg-white transition-colors
                ${error ? 'border-[#EF4444] text-[#EF4444]' : 'border-[#e0eaf1] focus:border-[#0B7BFF]'}`} />
            {error && <p className="text-[12px] text-[#EF4444] mt-1">{error}</p>}
          </div>
          <div>
            <label className="text-[12px] font-semibold text-[#0b2a3b] mb-1.5 block">Password</label>
        ...
        <p className="text-center text-[13px] text-[#6b7785] mt-5">
          Already have an account? <Link to="/login" className="text-[#0B7BFF] font-semibold">Sign in</Link>
        </p>
      </div>
    </div>
  );
